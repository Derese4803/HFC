...
def get_comprehensive_error_analysis(constraints_df: pd.DataFrame, logic_df: pd.DataFrame) -> Dict:
    ...
    analysis = {
        'error_type_overview': {}, 'error_rate_by_enumerator': pd.DataFrame(),
        'enumerators_without_errors': [], 'most_common_variables': {},
        'strange_values': pd.DataFrame(), 'overall_stats': {}
    }
    
    all_errors = []
    if constraints_df is not None and len(constraints_df) > 0:
        c_copy = constraints_df.copy()
        c_copy['error_category'] = 'Constraint'
        all_errors.append(c_copy)
    if logic_df is not None and len(logic_df) > 0:
        l_copy = logic_df.copy()
        l_copy['error_category'] = 'Logic'
        all_errors.append(l_copy)
        
    if not all_errors: return analysis
    combined_errors = pd.concat(all_errors, ignore_index=True)
    all_enumerators = set(combined_errors['username'].unique())
    ...
    analysis['error_type_overview'] = {
        ...
        'Unique Farmers Affected': combined_errors[id_col].nunique() if id_col else 0
    }
    
    enum_analysis = []
    existing_corrections = load_existing_corrections()
    for enum in all_enumerators:
        e_errs = combined_errors[combined_errors['username'] == enum]
        solved = len(existing_corrections[existing_corrections['corrected_by'] == enum]) if existing_corrections is not None else 0
        enum_analysis.append({
            'Username': enum, 'Total Errors': len(e_errs), 'Solved': solved, 'Remaining': len(e_errs) - solved
        })
    analysis['error_rate_by_enumerator'] = pd.DataFrame(enum_analysis)
    analysis['overall_stats'] = {'Total Enumerators': len(all_enumerators), 'Unique Variables with Errors': combined_errors['variable'].nunique()}
    return analysis
...
def validate_farmer_corrections(farmer_id: str, id_col: str) -> Tuple[bool, List[str], int, int]:
    ...
    for error_key, correction_data in farmer_corrections.items():
        ...
        if not explanation:
            missing.append(f"Missing justification comment.")
            continue
        completed += 1
...
# ============================================================================
# MAIN APPLiCATION ROUTER & CORE ORCHESTRATION WORKFLOW
# ============================================================================

def main():
    st.title("🌾 HFC Structural Field-Data Correction System")
    
    # Verify secure cloud connections
    if not check_token_validity():
        return

    # Phase 1: Retrieve raw data sheets from GitHub
    ...
    # Phase 2: Administrative Reporting Routing Portal
    ...
    # Route UI to Admin Analytics Window or Regular Enumerator Forms Window
    ...
    # Step 3: Enumerator Workspace Setup
    ...
    if selected_enum == "-- Select ID --":
        st.info("Please select your assigned username identifier from the selector menu to populate your list of assigned error corrections.")
        return

    # Extract pending entries for selected worker
    ...
    # Combine errors under target Farmers
            ...
            if st.button("Commit Correction", key=f"btn_{key}"):
                ...
                if data and data['explanation'].strip():
                    new_row_df = pd.DataFrame([{
                        'error_type': 'constraint', 'unique_id': row[id_col], 'variable': row['variable'],
                        'correct_value': data['correct_value'], 'explanation': data['explanation'],
                    ...
                    if save_corrections_to_github(new_row_df):
                        ...
                        st.rerun()
                else: st.error("Justification text required.")

    # Process Logic Discrepancies
    for idx, row in l_pending.iterrows():
        ...
        with st.expander(f"⚠️ System Mismatch Discrepancy: ID {row[id_col]} ({row['variable']})"):
            ...
            if st.button("Commit Correction", key=f"btn_{key}"):
                ...
                if data and data['explanation'].strip():
                    new_row_df = pd.DataFrame([{
                        'error_type': 'logic', 'unique_id': row[id_col], 'variable': row['variable'],
                        'correct_value': data['correct_value'], 'explanation': data['explanation'],
                        'corrected_by': selected_enum, 'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    }])
                    if save_corrections_to_github(new_row_df):
