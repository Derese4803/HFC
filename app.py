...
if st.session_state.get('data') is not None:
    ...
    for idx, row in pending.iterrows():
        with st.container():
            ...
            # UNIQUE KEY FIX: Added _idx to button key
            if st.button(f"Fix {row[id_col]}", key=f"btn_{row[id_col]}_{idx}"):
                st.session_state.target = row[id_col]
                st.rerun()
                
    # --- CORRECTION ENTRY FORM ---
    if 'target' in st.session_state:
        st.write(f"### Correcting Record: {st.session_state.target}")
        val = st.text_input("Corrected Value", key="new_val")
